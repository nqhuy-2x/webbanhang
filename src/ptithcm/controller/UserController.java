...
//					getSHAHash(user.getMatKhau())
	...
	{
		boolean check = true;
		// kiểm tra định dạng email : nqhuy20042001@gmail.com.vn
		if (to.trim().isEmpty()) {
			check = false;
			model.addAttribute("LoiDinhDangEmail", "Email không được để trống");
		...
		if (check) {
			try {
				...
				if (user == null) {
					...
					s.update(user);
					String from = "n19dcat037@student.ptithcm.edu.vn";
					String subject = "FOOD DAY - Quên mật khẩu";
					String body="Click vào đường link sau để tạo mật khẩu mới: <a name=\"token\" href=\"http://localhost:8080/WebBanHang/user/create-new-pass.htm?token="+user.getToken()+"&email="+user.getEmail()+"\">Tạo mật khẩu mới</a>";
					System.out.println(token);
